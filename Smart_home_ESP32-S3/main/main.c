...
void i2s_driver_init(void)
{
    ...
    ESP_LOGI(TAG, "I2S Audio driver initialized.");
}
...
void audio_loopback_task(void *args)
{
    ...
    int32_t *data_buf = (int32_t *)malloc(BUF_SIZE);
    
    while (1)
    {
        // 阻塞式读取
        if (i2s_channel_read(rx_handle, data_buf, BUF_SIZE, &bytes_read, portMAX_DELAY) == ESP_OK)
        {
            int16_t *send_ptr = (int16_t *)data_buf;
            int32_t *recv_ptr = (int32_t *)data_buf;
            // 处理数据：32bit 转 16bit，移位减少点（11-12位声音更大）
            for (int i = 0; i < bytes_read / 4; i++)
            {
                send_ptr[i] = (int16_t)(recv_ptr[i] >> 12); 
            }
            i2s_channel_write(tx_handle, send_ptr, bytes_read / 2, &bytes_written, portMAX_DELAY);
        }
    }
}
...
void mqtt_app_start(void)
{
    esp_mqtt_client_config_t mqtt_cfg = {
        .broker.address.uri = HUAWEI_MQTT_URL, .broker.address.port = HUAWEI_MQTT_PORT,
        .credentials.client_id = HUAWEI_CLIENT_ID, .credentials.username = HUAWEI_USERNAME, .credentials.authentication.password = HUAWEI_PASSWORD,
    };
